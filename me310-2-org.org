...
return round(rev_per_sec_a,2), round(rpm_a,1), round(r_b,3), m, round(z_B,1), round(tension_diff)
   ...
   #+BEGIN_SRC latex :result value :var ang1=angular_velocity[0] ang2=angular_velocity[1]
   \begin{align*}
...
2. Assuming no power loss, the power the belt-pulley system needs to transfer is $P = T\omega = 2(500)(2\pi/60)$ = src_python[:results raw]{return(round(2*500*2*3.1416/60,1))} W.
...
1. To design the shaft, we must first determine the loads on the shaft, which are exerted by spur gear \(B\) and the pulley. Let us determine the load from each component.
...
2. Since the problem is not symmetrical, we need to determine the
